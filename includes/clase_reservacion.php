...
class Reservacion extends ConexionMYSql
{
    ...
    public function mostrar_salidas($posicion, $id, $inicio_dia="", $fin_dia=""){
        ...
        LEFT JOIN tipo_hab ON hab.tipo = tipo_hab.id
        INNER JOIN forma_pago ON reservacion.forma_pago = forma_pago.id
        WHERE  movimiento.finalizado>= $inicio_dia AND   movimiento.finalizado<=  $fin_dia  AND  movimiento.finalizado >0 AND hab.id>0 ORDER BY hab.id ASC"; //echo $sentencia;
        $cat_paginas=($this->total_elementos($sentencia)/20);
        $extra=($this->total_elementos($sentencia)%20);
        $cat_paginas=intval($cat_paginas);
        if($extra>0) {
                ...
                <div class="card">
                    <div class="card-header" id="heading'.+$nreserva.'">
                        <div class="toogle-button"  data-toggle="collapse" data-target="#collapse'.+$nreserva.'" aria-expanded="true" aria-controls="collapse'.+$nreserva.'">
                            <p class=" header_espacio ">'.$nreserva.'.- <strong> '.$fila['persona'].' '.$fila['apellido'].' </strong></p>
                            <p class=" header_espacio fst-italic"><span class="fw-bold">Del: </span>'.date("d-m-Y", $fila['fecha_entrada']).' <span class="fw-bold">Al: </span>'.date("d-m-Y", $fila['fecha_salida']).'</p>
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class=" header_espacio bi bi-caret-down-fill" viewBox="0 0 16 16">
    ...
    public function buscar_entradas_salidas_recep($a_buscar, $id, $inicio_dia, $opcion, $fin_dia, $radio)
    {
        ...
        if($opcion == 1) {
            ...
            if($radio=="salidas"){
                ...
                LEFT JOIN tipo_hab ON hab.tipo = tipo_hab.id
                INNER JOIN forma_pago ON reservacion.forma_pago = forma_pago.id
                WHERE  ((movimiento.fin_hospedaje>= $inicio_dia AND   movimiento.fin_hospedaje<=  $fin_dia)OR (movimiento.finalizado>= $inicio_dia AND   movimiento.finalizado<=  $fin_dia))  AND hab.id>0 ORDER BY hab.id ASC";
            }
    ...
    public function mostrar_reservacion_fecha($fecha_ini_tiempo, $fecha_fin_tiempo, $a_buscar, $combinada, $id)
    {
        ...
        if($a_buscar == ' ' && strlen($fecha_ini) == 0 && strlen($fecha_fin) == 0) {
            ...
            if($a_buscar != ' ' && $noexiste_inicio && $noexiste_fin) {
                $sentencia = "SELECT *,reservacion.precio_hospedaje as precio_hospedaje_reserva,huesped.correo as correo_huesped, reservacion.tipo_hab as tipo_hab_numero, hab.nombre as nombre_hab, movimiento.id as mov,movimiento.id_hab,reservacion.id AS ID,tipo_hab.nombre AS habitacion,huesped.nombre AS persona,huesped.apellido,usuario.usuario AS usuario,reservacion.estado AS edo,huesped.telefono AS tel
                FROM reservacion
                LEFT JOIN tarifa_hospedaje  ON reservacion.tarifa = tarifa_hospedaje.id
                INNER JOIN movimiento ON reservacion.id = movimiento.id_reservacion
                LEFT JOIN tipo_hab ON tarifa_hospedaje.tipo = tipo_hab.id
    ...
    public function datos_por_dia($dia, $a_buscar)
    {
        ...
        echo '<div class="inputs_form_container justify-content-start">
        ...
        echo '<input type="text" id="a_buscar" placeholder="Buscar" class="form-control custom_input" autofocus="autofocus"/>
        ...
        echo '
            ...
            <div class="form-floating ">
                <button class="btn btn-primary btn-block" onclick="reporte_reservacion_por_dia('.$dia.')">
